/**
 * This library has been modified to work with visual editor linear data
 * instead of strings.
 */
...
diff_match_patch.prototype.arraysEqual = function (a, b) {
  ...
  for (var i = 0; i < a.length; ++i) {
    if (a[i] !== b[i] && !ve.dm.ElementLinearData.static.compareElements(a[i], b[i], this.oldStore, this.newStore)) {
      return false;
    }
  }
  return true;
...
diff_match_patch.prototype.diff_commonSuffix = function(text1, text2) {
  ...
  while (pointermin < pointermid) {
    ...
    if (this.arraysEqual(text1.slice(text1.length - pointermid, text1.length - pointerend),
